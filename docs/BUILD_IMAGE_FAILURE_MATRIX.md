...
- The build and runtime Dockerfiles still use floating Debian inputs such as `FROM debian:stable` and `apt-get dist-upgrade`.
...
| --- | --- | --- | --- | --- | --- |
| `webapp` | Shared venv built in `eduid-build`, then copied into runtime image | `eduid-prebuild` uses floating Debian for build-stage Python and build libs; runtime image uses floating Debian again | Python minor drift can fail locked install in shared build; wheel drift can force source builds for packages such as `cryptography`, `pillow`, `pyopenssl`, `pysaml2`, and `xhtml2pdf`; runtime Debian drift can break the copied venv later through interpreter or shared-library mismatch | Usually during `build/setup-venv.sh`; sometimes only at container start | Mostly hard at build time, but copied-venv ABI problems can be delayed until runtime |
...
1. Did the floating Debian base move Python away from 3.13?
2. Did the available wheel set change for the new Python or Debian combination?
3. Did a package fall back to source build and hit a missing or changed system dependency?
