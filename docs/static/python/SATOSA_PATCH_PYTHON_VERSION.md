...
- **build/setup-venv.sh**: The shared releng build now uses `uv pip install --require-hashes` with `uv` bootstrapped in the prebuild image, so the old mutable `pip install --upgrade pip wheel` issue no longer applies to the main shared build path even though `uv` itself is still unpinned
